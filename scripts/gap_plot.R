...
plot_gap_panels_mode <- function(df, sp, ssp_sel = "585", include_historical = TRUE) {
  ...
  # tight outer margins; we'll set inner margins per-panel
  op <- par(mfrow = c(1, length(periods)),
            oma = c(0.2, 0.5, 4, 0.5),
            xaxs = "i", yaxs = "i",
            mgp = c(2, 1.5, 0),
            cex.axis = 3)
  ...
  for (i in seq_along(periods)) {
    ...
    # per-panel inner margins: only the first plot gets a wide left margin for the y-axis label
    left_mar  <- if (i == 1) 6 else 1.2
    right_mar <- if (i == length(periods)) 1.2 else 0.8
    ...
    if (i == 1) {
      axis(2, at = seq(0, 100, 25), cex.axis = 2.15)
      mtext("In situ conservation score", side = 2, line = 3.9, cex = 2.3, font = 1)
    }
    axis(1, at = 1:4, labels = metrics, las = 1, cex.axis = 2.0, tck = 0)
    ...
    if (include_historical && yr == 2000 && any(rows$ssp == "historical")) {
      ...
      x <- 1:4
      y <- as.numeric(r[1, metrics])
      segments(x, 0, x, y, lwd = 6, col = metric_cols)
      points(x, y, pch = 15, cex = 4, col = metric_cols)
    }
    
    # Future: no-migration vs migration
    r_nomig <- rows %>% dplyr::filter(ssp == ssp_sel, mode == "no_migration")
    r_mig   <- rows %>% dplyr::filter(ssp == ssp_sel, mode == "migration")
    
    if (nrow(r_nomig) > 0) {
      x <- 1:4 - 0.12
      y <- as.numeric(r_nomig[1, metrics])
      segments(x, 0, x, y, lwd = 6, col = metric_cols)
      points(x, y, pch = 1, cex = 4, lwd = 3.5, col = metric_cols, bg = metric_cols)
    }
    
    if (nrow(r_mig) > 0) {
      x <- 1:4 + 0.12
      y <- as.numeric(r_mig[1, metrics])
      segments(x, 0, x, y, lwd = 6, col = metric_cols)
      points(x, y, pch = 19, cex = 4, col = metric_cols, bg = metric_cols)
    }
    par(xpd = FALSE)
    ...
    text(x = 4.25, y = 25, labels = "HP", col = "black", cex = 2.1, font = 1, adj = c(0, 0.5))
    
    segments(x0 = 0.55, y0 = 50, x1 = 4.25, y1 = 50, lty = 2, col = "black", lwd = 1.5)
    text(x = 4.25, y = 50, labels = "MP", col = "black", cex = 2.1, font = 1, adj = c(0, 0.5))
    
    segments(x0 = 0.55, y0 = 75, x1 = 4.25, y1 = 75, lty = 2, col = "black", lwd = 1.5)
    text(x = 4.25, y = 75, labels = "LP", col = "black", cex = 2.1, font = 1, adj = c(0, 0.5))
    
    par(xpd = FALSE)
    # Year label
    mtext(year_labels[i], side = 1, line = 6.2, cex = 2.2, font = 1)
    
    # Taxon lable
    if (sp == "fus") taxon_label <- expression(italic("Malus fusca"))
    if (sp == "cor") taxon_label <- expression(italic("Malus coronaria"))
    if (sp == "ion") taxon_label <- expression(italic("Malus ioensis"))
    if (sp == "ang") taxon_label <- expression(italic("Malus angustifolia"))
    if (sp == "chl") taxon_label <- expression(Sect.~italic("Chloromeles"))
...
# Example usage
png("C:/Users/terre/Documents/Acadia/Malus Project/statistical analysis/gap_analysis/gap_analysis_TEST.png", width = 6000, height = 2000, res = 300)
...
make_grid_for_ssp <- function(ssp_dir) {
  ...
  g_leg_v   <- arrangeGrob(g_leg_mig, g_leg_gap, ncol = 1, heights = c(1, 1))
